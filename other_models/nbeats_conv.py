...
class NBeatsBlock(t.nn.Module):
    ...
    def __init__(self,
        ...
        # Add a convolutional layer before the fully connected layers
        self.conv = t.nn.Conv1d(in_channels=1, out_channels=1, kernel_size=3)
    ...
    def forward(self, x: t.Tensor) -> Tuple[t.Tensor, t.Tensor]:
        ...
        #CONV MOD
        # x is batch_size x dates
        # conv1d needs batch_size x channels = 1 x dates
        x = x.unsqueeze(1)
        x = self.conv(x)
        # block_input is batch_size x dates
        # Remove the channel dimension
        block_input = x.squeeze(1)
        # pad the input with 0s
        block_input = t.nn.functional.pad(block_input, (1, 1), "constant", 0)
