...
#Clamp the weights of the attention layer
def clamp_weights(self, min_value, max_value):
    self.in_proj_weight.data.clamp_(min_value, max_value)
    self.out_proj.weight.data.clamp_(min_value, max_value)
    if self.bias_k is not None:
        self.bias_k.data.clamp_(min_value, max_value)
    if self.bias_v is not None:
        self.bias_v.data.clamp_(min_value, max_value)
    if self.in_proj_bias is not None:
        self.in_proj_bias.data.clamp_(min_value, max_value)
        
MultiheadAttention.clamp_weights = clamp_weights
...
class NBeatsBlock(t.nn.Module):
    ...
    def __init__(self,
        ...
        """
        super().__init__()
        # Define multi-headed attention layer
    ...
    def forward(self, x: t.Tensor) -> Tuple[t.Tensor, t.Tensor]:
        block_input, _ = self.attention(query = x, key = x, value = x, need_weights = False)
